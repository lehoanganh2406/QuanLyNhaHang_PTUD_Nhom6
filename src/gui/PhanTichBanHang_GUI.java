...
public class PhanTichBanHang_GUI extends JPanel {
    ...
    private JPanel createHeaderPanel() {
        ...
        JLabel lblSub = new JLabel("So sánh doanh thu, chi phí ước tính và lợi nhuận theo khoảng ngày");
    ...
    private void loadData() {
        ...
        double cost = revenue * 0.60; // SQL hiện chưa có cột giá vốn/nguyên liệu, nên lấy ước tính 60% doanh thu.
    ...
    private List<ItemSale> loadItemSales(LocalDate start, LocalDate end) {
        ...
        try {
            ...
            String sql = "SELECT m.maMon, m.tenMon, ISNULL(SUM(ct.soLuong - ISNULL(ct.soLuongHuy, 0)), 0) AS soLuong, "
            ...
            while (rs.next()) {
                ...
                item.chiPhi = item.doanhThu * 0.60;
                item.loiNhuan = item.doanhThu - item.chiPhi;
                list.add(item);
            }
        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Lỗi tải dữ liệu phân tích: " + e.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
        } finally {
            closeQuietly(rs);
            closeQuietly(stmt);
        }
    ...
    private List<PeriodSummary> loadPeriodSummary(LocalDate start, LocalDate end) {
        ...
        try {
            con = ConnectDB.getConnection();
            if (con == null) return list;
            String sql = "SELECT CAST(hd.thoiGianRa AS DATE) AS ngay, ISNULL(SUM(hd.tongTien), 0) AS doanhThu "
                    + "FROM HoaDon hd "
                    + "WHERE hd.trangThai = N'Đã thanh toán' "
                    + "AND hd.thoiGianRa >= ? AND hd.thoiGianRa < ? "
            ...
            while (rs.next()) {
                ...
                p.chiPhi = p.doanhThu * 0.60;
                p.loiNhuan = p.doanhThu - p.chiPhi;
                list.add(p);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void updateTable(List<ItemSale> items) {
        ...
        int stt = 1;
        for (ItemSale item : items) {
            String note = item.loiNhuan >= item.doanhThu * 0.35 ? "Lãi cao" : item.loiNhuan >= 0 ? "Ổn định" : "Cần kiểm tra";
    ...
    private String buildNhanXet(List<ItemSale> items, double revenue, double profit, double margin, int totalQty, LocalDate start, LocalDate end) {
        ...
        return "• Tổng số lượng món bán: " + totalQty + "\n"
                + "• Món có doanh thu cao nhất: " + top + "\n"
                + "• Lợi nhuận " + (profit >= 0 ? "dương" : "âm") + ", tỷ suất khoảng " + String.format("%.1f%%", margin) + "\n"
                + "• Chi phí nguyên liệu đang ước tính 60% doanh thu do SQL chưa có bảng giá vốn.";
    }
    ...
    class MockChartPanel extends JPanel {
        ...
        public MockChartPanel() { setOpaque(false); }
        ...
        @Override protected void paintComponent(Graphics g) {
            ...
            int legY = 15;
            int cx1 = 70;
            g2.setColor(new Color(110, 170, 230)); g2.fillRect(cx1, legY - 10, 14, 14);
            g2.setColor(Color.GRAY); g2.drawString("Doanh thu", cx1 + 22, legY + 2);
            int cx2 = cx1 + 120;
            g2.setColor(new Color(140, 200, 130)); g2.fillRect(cx2, legY - 10, 14, 14);
            g2.setColor(Color.GRAY); g2.drawString("Chi phí", cx2 + 22, legY + 2);
            int cx3 = cx2 + 100;
            g2.setColor(new Color(250, 180, 100)); g2.setStroke(new BasicStroke(3f)); g2.drawLine(cx3, legY - 3, cx3 + 20, legY - 3);
            g2.setColor(Color.GRAY); g2.drawString("Lợi nhuận", cx3 + 28, legY + 2);
